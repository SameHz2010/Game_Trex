...
void SH1106_ClearArea(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, SH1106_COLOR_t color) {
    // Input validation: ensure coordinates are within the display bounds and x1 <= x2, y1 <= y2
    if (x1 > x2 || y1 > y2 || x2 >= SH1106_WIDTH || y2 >= SH1106_HEIGHT) {
        return; // Exit if coordinates are invalid
    }

    // Iterate through the specified rectangular area
    for (uint8_t y = y1; y <= y2; y++) {
        for (uint8_t x = x1; x <= x2; x++) {
            // If the color is black, clear the pixel (set it to 0)
            if (color == SH1106_COLOR_BLACK) {
                SH1106_DrawPixel(x, y, SH1106_COLOR_BLACK); // Clear pixel (black)
            }
        }
    }

    SH1106_UpdateScreen();  // Update screen to reflect the changes
}
